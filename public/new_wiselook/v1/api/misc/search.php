...
$search_type = isset($input['search_type']) ? $input['search_type'] : 'all';
...
if (!in_array($search_type, ['posts', 'users', 'all'])) {
    echo json_encode(['success' => false, 'message' => 'Invalid search type. Must be "posts", "users" or "all"']);
    exit;
}
    ...
    if ($search_type === 'users' || $search_type === 'all') {
        ...
        foreach ($users as $user) {
            $processed_users[] = [
                ...
                'bio' => $user['bio'],
                'type' => $user['type'],
                'is_friend' => ($user['type'] === 'add')
            ];
        }
    }

    // Prepare response based on search type
    if ($search_type === 'all') {
        echo json_encode([
            'success' => true,
            'type' => 'all',
            'posts_count' => count($processed_posts),
            'users_count' => count($processed_users),
