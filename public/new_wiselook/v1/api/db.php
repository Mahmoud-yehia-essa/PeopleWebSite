...
define('DB_USER', 'u425698010_user');
define('DB_PASS', 'B^no4sKd1d2:');
define('DB_NAME', 'u425698010_new_wiselook');
// define('DB_PORT', '5011');
    ...
    private function connect()
{
  ...
  public function checkStoryExists($story_id)
  {
    $query = "SELECT id FROM `stories` WHERE id = '$story_id' AND is_active = 1";
    $result = $this->getResults($query);

    if (empty($result)) {
      $response = [
        'success' => false,
        'message' => "Story doesn't exist or has been deleted."
      ];
      echo json_encode($response);
      exit;
    }
...
//   public function timeAgo($datetime , $lang)
//   {
...
public function timeAgo($datetime, $lang = "ar")
{
    $timestamp = strtotime($datetime);
    $currentTime = time();
    $timeDiff = $currentTime - $timestamp;
    ...
    if ($lang == 'ar') {
        if ($seconds < 60) {
            ...
            return "منذ " . $this->arabicNatural($minutes, "دقيقة", "دقيقتين", "دقائق");
        } elseif ($hours < 24) {
            return "منذ " . $this->arabicNatural($hours, "ساعة", "ساعتين", "ساعات");
        } elseif ($days < 7) {
            return "منذ " . $this->arabicNatural($days, "يوم", "يومين", "أيام");
        } elseif ($weeks < 4) {
            return "منذ " . $this->arabicNatural($weeks, "أسبوع", "أسبوعين", "أسابيع");
        } elseif ($months < 12) {
            return "منذ " . $this->arabicNatural($months, "شهر", "شهرين", "أشهر");
        } else {
            return "منذ " . $this->arabicNatural($years, "سنة", "سنتين", "سنوات");
        }
    } else {
        if ($seconds < 60) {
            return "Just now";
...
private function arabicNatural($number, $singular, $dual, $plural)
{
    if ($number == 1) {
        return $singular;
    } elseif ($number == 2) {
