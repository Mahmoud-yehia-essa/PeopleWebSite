...
try {
    // For posts, check if it's a shared post (has parent_id)
    $parent_id = null;
    ...
    if ($content_type === 'post') {
        ...
        // If this post was a share (has parent_id), decrement the original post's share_count
        if ($parent_id) {
            $decrementShare = $conn->prepare("UPDATE posts SET share_count = share_count - 1 WHERE id = :parent_id");
            $decrementShare->bindParam(':parent_id', $parent_id, PDO::PARAM_INT);
            $decrementShare->execute();
        }
