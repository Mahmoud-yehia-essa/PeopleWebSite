...
// Process mentioned_user_ids
$mentioned_user_ids = [];
if (isset($_POST['mentioned_user_ids']) && !empty($_POST['mentioned_user_ids'])) {
    $mentioned_user_ids = json_decode($_POST['mentioned_user_ids'], true);
    if (!is_array($mentioned_user_ids)) {
        $mentioned_user_ids = [];
    }
    // Sanitize user IDs
    $mentioned_user_ids = array_map('intval', $mentioned_user_ids);
    $mentioned_user_ids = array_filter($mentioned_user_ids); // Remove empty values
}
...
try {
    ...
    if (!empty($mentioned_user_ids)) {
        ...
        foreach ($mentioned_user_ids as $mentioned_user_id) {
            ...
            $stmtCheckUser = $conn->prepare("SELECT id FROM users WHERE id = :user_id");
            $stmtCheckUser->bindParam(':user_id', $mentioned_user_id);
            $stmtCheckUser->execute();
            
            if ($stmtCheckUser->rowCount() > 0) {
                $stmtMention = $conn->prepare("INSERT INTO mentions 
                                             (content_type_id, content_id, user_id, created_at) 
                                             VALUES (:content_type_id, :content_id, :user_id, NOW())");
                $stmtMention->bindParam(':content_type_id', $content_type_id);
                $stmtMention->bindParam(':content_id', $post_id);
                $stmtMention->bindParam(':user_id', $mentioned_user_id);
                
                if (!$stmtMention->execute()) {
                    // Log the error but don't fail the entire post creation
                    error_log("Failed to insert mention for user $mentioned_user_id in post $post_id");
                }
            }
        }
        
        $response['data']['mentions'] = [
            'mentioned_count' => count($mentioned_user_ids),
