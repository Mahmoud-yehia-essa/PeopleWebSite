...
// Main posts query for saved posts
$sql = "
    SELECT 
        p.*,
    ...
    WHERE p.is_active = 1 AND p.id IN (" . implode(',', array_fill(0, count($saved_post_ids), '?')) . ")
    ORDER BY FIELD(p.id, " . implode(',', $saved_post_ids) . ")
";

$stmt = $conn->prepare($sql);
$stmt->execute($saved_post_ids);
...
foreach ($posts as $post) {
    $processed_post = [
        ...
        'is_saved' => true, // All posts in this list are saved by definition
