...
// Fetch posts with block filtering
$sql = "
    SELECT 
        p.*,
...
// Prepare posts output with media array
$processed_posts = [];
foreach ($posts as $post) {
    $media = [];
    ...
    if (!empty($post['parent_id'])) {
        $original_media = [];
        if (!empty($post['original_image'])) $original_media[] = ['path' => $uploadsPath . $post['original_image'], 'type' => 'image'];
        if (!empty($post['original_video'])) $original_media[] = ['path' => $uploadsPath . $post['original_video'], 'type' => 'video'];
