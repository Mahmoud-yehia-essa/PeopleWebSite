...
// Define external upload server URL
$uploadFunction = 'https://amcserver.com/projects/upload_media.php'; // ✅ Update this to your actual upload URL
...
if ($hasMedia) {
    ...
    if ($uploadResponse === false || $httpCode !== 200) {
        $response['message'] = 'Failed to upload media to external server';
        $response['error_detail'] = $curlError;
        echo json_encode($response);
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        if (count($uploadedMedia) > 1) {
            for ($i = 1; $i < count($uploadedMedia); $i++) {
                ...
                $img = $media['type'] === 'image' ? $media['name'] : null;
                $vid = $media['type'] === 'video' ? $media['name'] : null;
                ...
                $stmtMedia->bindParam(':image', $img);
                $stmtMedia->bindParam(':video', $vid);
                $stmtMedia->execute();
            }
        }
