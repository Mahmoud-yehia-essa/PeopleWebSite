...
// Insert story with expires_at calculated in DB
try {
    // Modified query to include expires_at (24 hours from now)
    $stmt = $conn->prepare("INSERT INTO stories 
                          (user_id, content, image, video, expires_at) 
                          VALUES (:user_id, :content, :image, :video, DATE_ADD(NOW(), INTERVAL 24 HOUR))");
    ...
    if ($stmt->execute()) {
        ...
        // Fetch the complete inserted story including expires_at
        $fetchStmt = $conn->prepare("SELECT *, DATE_ADD(created_at, INTERVAL 24 HOUR) AS expires_at FROM stories WHERE id = :id");
        $fetchStmt->bindParam(':id', $story_id);
        $fetchStmt->execute();
        $story = $fetchStmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($story) {
            ...
            $response['message'] = 'Story created but could not be retrieved';
        }
    } else {
