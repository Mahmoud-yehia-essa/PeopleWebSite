...
include_once __DIR__ . '/v1/api/config.php';
...
function uploadToLocalServer($files, $additionalFields = []) {
    $response = [
        'success' => false,
        'message' => '',
        'media_name' => [],
    ...
    // Normalize files array if it's a single file
    if (!is_array($files['name'])) {
        $files = [
            'name' => [$files['name']],
            'type' => [$files['type']],
    ...
    $uploadedFiles = [];
    $errors = [];
    $maxSize = 200 * 1024 * 1024; // 10MB max
    ...
    foreach ($files['name'] as $key => $name) {
        ...
        if ($size > $maxSize) {
            $errors[] = [
                'filename' => $name,
                'error' => 'File is too large (max 10MB allowed)'
            ];
            continue;
        }

        $sanitizedName = preg_replace('/[^a-zA-Z0-9\-\._]/', '', $name);
        $sanitizedName = time() . '_' . $sanitizedName;
    ...
    $response['media_name'] = $uploadedFiles;
    $response['errors'] = $errors;
