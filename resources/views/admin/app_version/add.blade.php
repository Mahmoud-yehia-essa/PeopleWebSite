    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">إضافة إصدار جديد</li>
            </ol>
        </nav>
    </div>
</div>
        ...
        <div class="card border-top border-0 border-4 border-primary shadow-sm">
            <div class="card-body p-5">
                <div class="card-title d-flex align-items-center mb-4">
                    ...
                    <h5 class="mb-0 text-primary font-weight-bold">إضافة إصدار جديد للتطبيق</h5>
                </div>
                <hr>
                
                <form action="{{ route('admin.app_versions.store') }}" method="POST" class="row g-3">
                    @csrf
                    
                    <!-- 1. رقم الإصدار -->
                    <div class="col-md-6">
                        <label for="version" class="form-label font-weight-bold text-dark">رقم الإصدار <span class="text-danger">*</span></label>
                        <input type="text" name="version" class="form-control @error('version') is-invalid @enderror" id="version" placeholder="مثال: 1.0.0 أو 2.1.4" required value="{{ old('version') }}">
                        @error('version')
                    ...
                    <div class="col-md-6 d-flex align-items-center pt-4 ps-md-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="update_required" id="update_required" value="1" {{ old('update_required') ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="url" name="android" class="form-control" id="android" placeholder="https://play.google.com/store/apps/..." value="{{ old('android') }}">
                    </div>

                    <!-- 4. رابط تحميل آي أو إس -->
                    <div class="col-md-6">
                        <label for="ios" class="form-label font-weight-bold text-dark">رابط تطبيق الآيفون (App Store)</label>
                        <input type="url" name="ios" class="form-control" id="ios" placeholder="https://apps.apple.com/app/..." value="{{ old('ios') }}">
                    </div>

                    <!-- 5. الدعم / التواصل -->
                    <div class="col-12">
                        <label for="contact" class="form-label font-weight-bold text-dark">بيانات التواصل والدعم الفني</label>
                        <input type="text" name="contact" class="form-control" id="contact" placeholder="مثال: بريد الدعم أو رقم واتساب أو رابط مركز المساعدة" value="{{ old('contact') }}">
                    </div>

                    <!-- 6. وصف التحديث -->
                    <div class="col-12">
                        <label for="des" class="form-label font-weight-bold text-dark">ما الجديد في هذا الإصدار (وصف التحديث)</label>
                        <textarea name="des" class="form-control" id="des" rows="5" placeholder="اكتب الميزات والتحسينات المضافة في هذا التحديث...">{{ old('des') }}</textarea>
                    </div>

                    <!-- أزرار الإجراءات -->
                    <div class="col-12 text-end mt-4">
                        <a href="{{ route('admin.app_versions.index') }}" class="btn btn-secondary px-4 me-2">إلغاء</a>
                        <button type="submit" class="btn btn-primary px-4">حفظ الإصدار الجديد <i class="bx bx-save ms-1"></i></button>
                    </div>
                </form>
            </div>
        </div>
