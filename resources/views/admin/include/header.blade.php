        ...
        <nav class="navbar navbar-expand gap-3">
                            ...
                            <a href="javascript:;">
                                <div class="text-center msg-footer">
                                    ...
                                    <button class="btn btn-primary w-100">Checkout</button>
                                            ...
                                            $ncount = 0;
                                            @endphp
                    <!-- أيقونة الدعم الفني والتواصل المباشر -->
                    <li class="nav-item dropdown dropdown-large">
                        <a class="nav-link position-relative" href="{{ route('admin.support_tickets.index') }}" title="تذاكر الدعم الفني">
                            <i class="bx bx-message-square-detail"></i>
                            @php
                                $openTicketsCount = \App\Models\SupportTicket::where('status', 'open')->count();
                            @endphp
                            @if($openTicketsCount > 0)
                                <span class="alert-count">{{ $openTicketsCount }}</span>
                            @endif
                        </a>
                    </li>
            ...
            <div class="user-box dropdown px-3">
                <a class="d-flex align-items-center nav-link dropdown-toggle gap-3 dropdown-toggle-nocaret" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src="{{ (!empty(Auth::user()?->photo)) ? (filter_var(Auth::user()->photo, FILTER_VALIDATE_URL) ? Auth::user()->photo : url('new_wiselook/uploads/'.Auth::user()->photo)) : url('upload/no_image.jpg') }}" class="user-img" alt="user avatar">
                    <div class="user-info">
                        <p class="user-name mb-0">{{ Auth::user()?->fname ?? 'المدير' }}</p>
