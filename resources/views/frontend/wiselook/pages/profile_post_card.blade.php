@php
    $postUser = $post->user;
    $postFullName = $postUser ? trim($postUser->first_name . ' ' . $postUser->last_name) : 'مستشار حكمة';
    $postAvatarUrl = url('upload/no_image.jpg');
    if ($postUser && $postUser->profile_picture && $postUser->profile_picture !== 'non') {
        $postAvatarUrl = filter_var($postUser->profile_picture, FILTER_VALIDATE_URL)
            ? $postUser->profile_picture
            : asset('new_wiselook/uploads/' . $postUser->profile_picture);
    }
@endphp
...
<article class="bg-surface-container-lowest/70 backdrop-blur-[20px] rounded-xl border border-primary/10 p-6 shadow-sm ambient-shadow-low hover:ambient-shadow-mid transition-all duration-300">
    ...
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-4 space-x-reverse">
            <div class="relative">
                @if($postUser)
                    <a href="{{ route('profile.edit', $postUser->id) }}">
                        <img alt="{{ $postFullName }}" class="w-14 h-14 rounded-full object-cover border-2 border-outline-variant hover:opacity-90 transition-opacity" src="{{ $postAvatarUrl }}"/>
                    </a>
                @else
                    <img alt="{{ $postFullName }}" class="w-14 h-14 rounded-full object-cover border-2 border-outline-variant" src="{{ $postAvatarUrl }}"/>
                @endif
            ...
            <div>
                @if($postUser)
                    <a href="{{ route('profile.edit', $postUser->id) }}" class="hover:underline block">
                        <h3 class="font-headline-lg-mobile text-base md:text-base text-primary font-bold">{{ $postFullName }}</h3>
                    </a>
                @else
                    <h3 class="font-headline-lg-mobile text-base md:text-base text-primary font-bold">{{ $postFullName }}</h3>
                @endif
                <div class="flex items-center text-on-surface-variant font-label-sm text-label-sm space-x-2 space-x-reverse" style="margin-top: 8px;">
                    @if($postUser && $postUser->rank)
                        @php
                            $rankPhoto = $postUser->rank->photo;
                        ...
                        <span class="inline-flex items-center gap-1.5" style="display: inline-flex; align-items: center; gap: 6px; vertical-align: middle;">
                            <img src="{{ $rankPhotoPath }}" alt="{{ __t($postUser->rank->rank_name) }}" style="width: 20px; height: 20px; object-fit: contain; margin-left: 2px;">
                            <span class="font-bold text-xs" style="color: #cda225; text-shadow: 0 1px 2px rgba(0,0,0,0.15);">{{ __t($postUser->rank->rank_name) }}</span>
                        </span>
                    @else
                        <span>{{ $postUser && $postUser->role == 'admin' ? 'مدير المنصة' : 'مستشار تقني' }}</span>
                    @endif
                </div>
                @if($postUser)
                    <div class="mt-1.5 flex items-center justify-start">
                        <button type="button" class="user-points-trigger inline-flex items-center gap-1 text-[10px] font-extrabold text-secondary hover:text-primary transition-colors cursor-pointer bg-secondary/5 hover:bg-secondary/10 px-2 py-0.5 rounded-full border border-secondary/10" data-user-id="{{ $postUser->id }}">
                            <span class="material-symbols-outlined text-[12px]" style="font-variation-settings: 'FILL' 1;">star</span>
                            <span>{{ __t('points') }}: {{ $postUser->points ?? 0 }}</span>
                        </button>
                    </div>
                @endif
            </div>
    ...
    <div class="mt-6 pt-4 border-t border-surface-variant flex items-center justify-between text-on-surface-variant">
        <div class="flex space-x-6 space-x-reverse">
            ...
            <button class="open-discussion-btn flex items-center space-x-2 space-x-reverse hover:text-primary transition-colors group"
                    ...
                    data-author-name="{{ $postFullName }}"
                    data-author-avatar="{{ $postAvatarUrl }}"
                    data-post-title="{{ $postFullName }}"
        ...
        <div class="flex items-center space-x-4 space-x-reverse">
            ...
            <button class="share-post-btn flex items-center space-x-2 space-x-reverse hover:text-primary transition-colors cursor-pointer" data-post-id="{{ $post->id }}" data-post-content="{{ rawurlencode($post->content) }}">
